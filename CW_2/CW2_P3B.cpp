...
void assign_vectors(int m, int n, double K, double R, double r, double sigma_sq, double delta_t, double* f, double* U, double* diag, double* upper, double* lower, double* U_prev)
    {
double h = R/(double)(m);

f[0] = f_0((double)(n)*delta_t,K,r)*(-delta_t*sigma_sq/2);
f[m-1] = u_exact(R,(double)(n)*delta_t,r,sigma_sq,K)*(-delta_t*r*m - delta_t*sigma_sq*pow(m,2));

for (int i=0; i<m; i++)
    {
    diag[i] = 1 + delta_t*r + delta_t*r*i + delta_t*sigma_sq*pow(i,2);
    U_prev[i] = U[i] + f[i];
    }

for (int i=0; i<m-1; i++)
    {
    lower[i] = - delta_t*sigma_sq*pow(i,2)/2.0;
    upper[i] = - delta_t*r*i - delta_t*sigma_sq*pow(i,2)/2.0;
    }
...
assign_vectors(m,0,K,R,r,sigma_sq,delta_t,f,U,diag,upper,lower,U_prev);
...
for(int n = 0; n<num_of_time_steps+1; n++)
    ...
    assign_vectors(m,n,K,R,r,sigma_sq,delta_t,f,U,diag,upper,lower,U_prev);
    }
...
for(int i =0; i<m; i++)
    {
    Ei = fabs(U[i] - u_exact(h*(i+1),T,r,sigma_sq,K));
